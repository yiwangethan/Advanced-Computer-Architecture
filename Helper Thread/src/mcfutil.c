...
#endif
{
    ...
    pthread_t thread; /*新创建的线程ID会被设置成thread指向的内存单元*/
    long checksum = 0; 
    
    for( node = root, stop = net->stop_nodes; node < (node_t*)stop; node++ )
        node->mark = 0;
    pthread_create(&thread, NULL, helper_thread, root);/* add a new thread of control to the current process*/
    
    root->potential = (cost_t) -MAX_ART_COST;
    tmp = node = root->child;
    while( node != root )
    {
        while( node )/*改为 prefetch*/
        {
            ...
            tmp = node;
            node = node->child;
            helper_thread_sync();/*flag decreaments by 1*/
        }
    ...
    pthread_cancel(thread);/*cacnel thread*/
